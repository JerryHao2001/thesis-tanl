...
@register_dataset
class SciERCJointERDataset(JointERDataset):
    name = 'scierc_joint_er'
    data_name = 'scierc_joint_er'  # directory under data/
    # Naturalized names (edit if you prefer different text prompts)
    natural_entity_types = {
        'Task': 'task',
        'Method': 'method',
        'Metric': 'metric',
...
class SciERCCorefDataset(CoNLL12CorefDataset):
    name = 'scierc_coref'
    data_name = 'scierc'        # same dir as ER; different cache name because dataset name differs
    # default_output_format = 'coref' inherited
    ...
    def load_data_single_split(self, split: str, seed: int = None):
        ...
        # Use same chunk args TANL expects for coref
        chunk_size = self.data_args.chunk_size if not self.is_eval else self.data_args.chunk_size_eval
        chunk_overlap = self.data_args.chunk_overlap if not self.is_eval else self.data_args.chunk_overlap_eval

        path = os.path.join(self.data_dir(), f'{split}.json')
        with open(path, 'r') as f:
            for doc_i, line in enumerate(f):
                raw = json.loads(line)
                # Flatten tokens and compute sentence offsets
                tokens = [tok for s in raw['sentences'] for tok in s]
                offsets = np.cumsum([0] + [len(s) for s in raw['sentences'][:-1]])
                ...
                for cluster in raw.get('clusters', []):
                    ...
                    for m in cluster:
                        if len(m) == 3:
                            # (sent_id, start, end)  -- end is inclusive in SciERC/DyGIE format
                            sid, s, e = m
                            start = offsets[sid] + s
                            end   = offsets[sid] + e + 1
                        elif len(m) == 2:
                            # (start, end) in DOC space -- end is inclusive
                            s, e = m
                            start = s
                            end   = e + 1
                        else:
                            raise ValueError(f"Unexpected coref mention shape: {m}")
                        # sanity guard against bad spans
                        if 0 <= start < end <= len(tokens):
                            mentions.append(Entity(start=start, end=end))
                    if len(mentions) >= 2:
                        groups.append(mentions)

                # chunking (exactly like CoNLL12)
                chunks, pos, chunk_id = [], 0, 0
                while pos < len(tokens):
                    chunk_tokens = tokens[pos:pos+chunk_size]
                    chunk_groups = []
                    for g in groups:
                        m = [Entity(start=m.start-pos, end=m.end-pos) for m in g
                             if m.start >= pos and m.end <= pos+chunk_size]
                        if len(m) >= 2:
                            chunk_groups.append(m)
                    ex = InputExample(
                        id=f'{split}-{doc_i}-{chunk_id}',
                        tokens=chunk_tokens,
                        offset=pos,
                        groups=chunk_groups,
                        document_id=f'{split}-{doc_i}',
                        chunk_id=chunk_id,
                    )
                    examples.append(ex)
                    chunks.append(ex)

                    if pos + chunk_size >= len(tokens): break
                    pos += max(1, chunk_size - chunk_overlap)
                    chunk_id += 1

                self.documents[f'{split}-{doc_i}'] = CorefDocument(
                    id=f'{split}-{doc_i}',
                    tokens=tokens,
                    groups=groups,
                    chunks=chunks,
                    chunk_centers=[ex.offset + len(ex.tokens)//2 for ex in chunks],
                )
        return examples